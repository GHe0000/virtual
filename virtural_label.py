...
class Virtural:
    ...
    def draw(self, layer):
        ...
        a = self.add_rot(np.array([yaw, pitch, roll/5]),layer.name,a)
        if layer.name == 'hand':
            a = self.add_rot(np.array([0,0,-mouse_theta+1.27932]),layer.name,a)
        # a = a @ scale(2,2,2) \
        #     @ rotate(0.5, axis=(0, 2)) \
        #     @ translate(0.4, 0, 0.2)
        a = a @ perspective()
...
def get_mouse_theta():
    ...
    if (window_pos[0] - x - 1) != 0:
        ...
        # mouse_theta = np.clip(mouse_theta,0,1.5)
...
if __name__ == '__main__':
    window = init_window()
    add_hotkey('f1',key_f1)
    add_hotkey('f2',key_f2)
    test_int()
    V = Virtural(inf_yaml='./Label/init_inf.yaml',\
                 shape_yaml='./Label/change_inf.yaml',\
                 test_yaml='./Label/test.yaml',\
                 others_yaml='./Label/others.yaml')
