...
# ---------- Vitural类定义 ----------
class Virtural:
    def __init__(self, inf_yaml, shape_yaml, debug_others_inf_yaml, file_t, size=(1024, 1024), pic_size=(1024, 1024)):
    ...
    def draw(self, layer):
        ...
        t_a = np.array([t_c1[8],t_c1[9],t_c1[7],1])
        t_z = t_a[2:3]
        t_z -= 0.1
        t_a[:2] *= t_z
        ...
        while not glfw.window_should_close(window):
            ...
            for layer in self.Layers:
                if layer.visual == True:
                    ...
                    self.draw(layer)
            # t_d1 = np.array([0,0,t_c1[7],1]) @ perspective() @ translate(t_c1[8], t_c1[9], 0) @ inperspective() @ perspective()
            # t_d1 = np.array([t_c1[8],t_c1[9],t_c1[7],1])
            glPointSize(3.0)
...
if __name__ == '__main__':
    window = init_window()
    V = Virtural(inf_yaml='./init_inf.yaml', \
                 shape_yaml='./change_inf2.yaml', \
                 debug_others_inf_yaml='./t.yaml', \
                 file_t='./file_t.yaml')
